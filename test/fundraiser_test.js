...
contract ("Fundraiser", accounts => {
  ...
  describe("making donation", () => {
    ...
    it("includes donation in myDonations", async () => {
      ...
      assert.equal(
        value, values[0], "values should match"
      );
  ...
  describe("withdrawing funds", () => {
    ...
    it("transfers balance to beneficiary", async () => {
      ...
      const newBeneficiaryBanance = await web3.eth.getBalance(beneficiary);
      const beneficiaryDifference = newBeneficiaryBanance - currentBeneficiaryBalance;

      assert.equal(
        newContractBalance, 0, "coutract should have a 0 banance"
      );
      assert.equal(
